      ...
      {variant === "horizontal" ? (
        ...
        <ul className={clsx("flex flex-col items-center gap-2", className)}>
          {techStack.map((tech) => (
            ...
            >
              {ICON_MAP[tech] || tech}
              <span className="pointer-events-none absolute top-1/2 left-full translate-x-2 -translate-y-1/2 rounded-2xl border border-white/50 bg-neutral-800/80 p-2 whitespace-nowrap opacity-0 backdrop-blur-md transition-all duration-300 group-hover:opacity-100">
                {tech}
              </span>
            </li>
          ))}
